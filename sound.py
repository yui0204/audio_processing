...
class Wavedata:
    ...
    def __init__(self, fs, sound_data, name, nbyte, norm = False, logger=1):
        """
        コンストラクタ
    ...
    def plot(self, ymin = -1.0, ymax = 1.0):
    ...
    def stft_plot(self, N = 2048, cmin = -100, cmax = -20):
    ...
    def fft(self, N = 2048, bins = 1024):
        ...
        :param N: STFTのフレーム幅(通常、20~40ms程度が良いらしい。ここでは2048を使用)
    ...
    def cepstrum(self, N = 2048, plot=False):
        ...
        :param N: STFTのフレーム幅(通常、20~40ms程度が良いらしい。が2048を使用)
    ...
    def extract(self, wavedata, extract_name = "_extract"):
        ...
        :param wavedata: wavedata(引く側)
        :return :フィルタリング後のwavedata
        """
    ...
    def write_wav_sf(self, dir=None, filename=None, bit=24):
        ...
        save_dir = "./" if dir is None else dir
        save_path = "{0}{1}".format(save_dir, save_file_name)
...
class Multiwave:
    ...
    def plot(self, ymin = -0.05, ymax = 0.05):
        """このメソッドは時系列データをグラフ表示するためのものです"""
...
class Feature:
    ...
    def __classification_report(self, y_pred, name, print=False):
        """このメソッドはロジスティック回帰を用いてクラス分類を行います
        ...
        # Precision Recallの情報表示　※表示のみしかできないため、通常は使用しない
        if print == True:
            print("-------------Results of", name, "-------------")
            print(classification_report(self.__y_test, y_pred, digits=4))
...
if __name__ == "__main__":    
    ...
    drums = WavfileOperate("./Saxophone181_0.wav").wavedata
